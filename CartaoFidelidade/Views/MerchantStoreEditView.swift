//
//  MerchantStoreEditView.swift
//  CartaoFidelidade
//
//  Formulário de edição de loja do lojista
//

import SwiftUI
import FirebaseAuth

struct MerchantStoreEditView: View {
    ...
    private func loadValidCities() {
        Task {
            do {
                ...
                print("❌ [MerchantStoreEditView] Erro ao carregar cidades: \(error.localizedDescription)")
            }
        }
    }
    ...
    private func submit() {
        ...
        guard let currentUser = Auth.auth().currentUser else {
            errorMessage = "Você precisa estar autenticado para editar uma loja"
            showError = true
            return
        }
        ...
        Task {
            do {
                let storeUpdateData = StoreUpdateData(
                ...
                try await StoresService.shared.updateStore(
                    storeId: store.id,
                    merchantId: currentUser.uid,
                    storeData: storeUpdateData
                )
                
                print("✅ [MerchantStoreEditView] Loja atualizada com sucesso: \(store.id)")
                ...
                print("❌ [MerchantStoreEditView] Erro ao atualizar loja: \(error.localizedDescription)")
                await MainActor.run {
                    loading = false
